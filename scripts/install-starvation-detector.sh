#!/bin/bash
# Installation script for TUNNEL starvation detector systemd service
# This script installs the systemd service and timer for automated starvation detection
...
echo "=== TUNNEL Starvation Detector Installation ==="
echo
...
if [ ! -d "$WORKSPACE_DIR/.beads" ]; then
    ...
    if [ ! -d "$WORKSPACE_DIR/.beads" ]; then
        ...
        echo "The detector will use the current working directory during execution"
    fi
fi
...
if [ "$MODE" = "timer" ] || [ "$MODE" = "both" ]; then
    ...
    sed "s|/usr/local/bin/tunnel|$TUNNEL_BIN|g" "$CONFIG_DIR/tunnel-starvation-detector@.service" \
        | sed "s|/home/coding|$HOME|g" \
        > "$SYSTEMD_USER_DIR/tunnel-starvation-detector@.service"

    # Install timer file
    cp "$CONFIG_DIR/tunnel-starvation-detector.timer" "$SYSTEMD_USER_DIR/"

    # Reload systemd daemon
    systemctl --user daemon-reload

    # Enable and start timer
    systemctl --user enable tunnel-starvation-detector.timer || {
        ...
        echo "  systemctl --user enable --now tunnel-starvation-detector.timer"
    }

    # Start timer if not already running
    if systemctl --user is-active --quiet tunnel-starvation-detector.timer; then
        echo -e "${GREEN}Timer is already running${NC}"
    else
        echo "Starting timer..."
        systemctl --user start tunnel-starvation-detector.timer
    fi
...
if [ "$MODE" = "daemon" ] || [ "$MODE" = "both" ]; then
    ...
    systemctl --user enable tunnel-starvation-detector.service || {
        echo -e "${YELLOW}Warning: Failed to enable service. You may need to log out and back in for user systemd services to work.${NC}"
        echo "To enable manually after logging back in:"
        echo "  systemctl --user enable --now tunnel-starvation-detector.service"
    }

    # Start service if not already running
    if systemctl --user is-active --quiet tunnel-starvation-detector.service; then
        echo -e "${GREEN}Daemon is already running${NC}"
    else
        echo "Starting daemon..."
        systemctl --user start tunnel-starvation-detector.service
    fi
...
if [ "$MODE" = "timer" ] || [ "$MODE" = "both" ]; then
    ...
    echo "  The starvation detector will run every 5 minutes automatically."
    echo
    echo "  # Check timer status"
    echo "  systemctl --user status tunnel-starvation-detector.timer"
    echo
    echo "  # View detection logs"
    echo "  journalctl --user -u tunnel-starvation-detector@*"
    echo
    echo "  # Run detection manually"
    echo "  tunnel starvation-detector --scheduled"
    echo
fi

if [ "$MODE" = "daemon" ] || [ "$MODE" = "both" ]; then
    echo "${BLUE}Daemon Mode:${NC}"
    echo "  The starvation detector is running continuously in the background."
    echo
    echo "  # Check service status"
    echo "  systemctl --user status tunnel-starvation-detector.service"
    echo
    echo "  # View service logs"
    echo "  journalctl --user -u tunnel-starvation-detector.service -f"
    echo
    echo "  # Restart service"
    echo "  systemctl --user restart tunnel-starvation-detector.service"
    echo
fi

echo "${BLUE}Manual Commands:${NC}"
echo "  # Run single detection check"
echo "  tunnel starvation-detector"
echo
echo "  # Run in dry-run mode (no changes applied)"
echo "  tunnel starvation-detector --dry-run"
echo
echo "  # Run with auto-repair disabled"
echo "  tunnel starvation-detector --auto-repair=false"
echo
echo "  # Run in daemon mode manually"
echo "  tunnel starvation-detector --daemon"
echo
echo "${YELLOW}To uninstall:$NC}"
if [ "$MODE" = "timer" ] || [ "$MODE" = "both" ]; then
    echo "  systemctl --user disable --now tunnel-starvation-detector.timer"
    echo "  rm $SYSTEMD_USER_DIR/tunnel-starvation-detector@.service"
    echo "  rm $SYSTEMD_USER_DIR/tunnel-starvation-detector.timer"
fi
if [ "$MODE" = "daemon" ] || [ "$MODE" = "both" ]; then
    echo "  systemctl --user disable --now tunnel-starvation-detector.service"
    echo "  rm $SYSTEMD_USER_DIR/tunnel-starvation-detector.service"
fi
